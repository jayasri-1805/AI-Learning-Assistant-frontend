...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
    ...
    return (
        <>              
            ...
            <aside
                className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:shrink-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
            ...
            <div className="px-3 py-4 border-t border-slate-200/60">
                <button
                    ...
                    className="group flex items-center gap-3 w-full px-4 py-2.5 text-sm font-semibold font-semibold text-slate-700 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all duration-200"
                >
                    <LogOut
                        size={18}
                        strokeWidth={2.5}
